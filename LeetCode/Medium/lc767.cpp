...
public:
    string reorganizeString(string S) {
        vector<pair<int,char>> count(26, {0, 'a'});
        for (char c: S)
            count[c - 'a'].first++;
        for (int i = 0; i < 26; i++)
            count[i].second = 'a' + i;
...
int main(int argc, char *argv[]) {
    ...
    /* Input */
    vector<string> S = {"aab", "aaab"};

    /* Output */
    vector<string> ret;
    for (int i = 0; i < S.size(); i++)
        ret.push_back(solution.reorganizeString(S[i]));

    /* Answer */
    vector<string> answer = {"aba", "aaab"};
