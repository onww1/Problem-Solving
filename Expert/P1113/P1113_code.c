...
char* get_memory_address(char atbl[100000000]) {
	unsigned long long addr = 0;
	for (int i = 99999984; i < 100000000; ++i) {
		addr <<= 4;
		addr |= atbl[i];
	}
	return (char*)addr;
...
{	
	unsigned long long addr = (unsigned long long)memory;
	int idx = 99999999;
	while (addr) {
		atbl[idx--] = addr & 0xf;
		addr >>= 4;
	}
	for (int i = 0; i < 195312; ++i) atbl[i] = 0xff;
	atbl[195312] = 0xf;
}
...
{	
	...
	for (int i = 0; i <= 195312; ++i) {
		if (atbl[i]) {
			int bit = atbl[i] & -atbl[i], tmp = bit;
			int cnt = 0;
			while (cnt < 8) {
				if (tmp & 1) {
					atbl[i] ^= bit;
					return get_memory_address(atbl) + 2048 * i + 256 * cnt;
				}
				cnt++;
				tmp >>= 1;
			}
...
{
	unsigned long long diff = ptr - get_memory_address(atbl);
	int box = diff / 2048;
	int pos = diff % 2048 / 256;
	atbl[box] ^= (1 << pos);
}