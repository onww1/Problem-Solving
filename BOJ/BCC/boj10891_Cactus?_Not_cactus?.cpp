...
int main(int argc, char *argv[]) {
    ...
    vector<int> group(n + 1, -INF), count(n + 1, 0);
    for (int i = 0; i < BCC.bcc_count; ++i) {
        for (pii& edge: BCC.bcc[i]) {
            if (group[edge.X] != i) group[edge.X] = i;
            else if (group[edge.X] == i) count[edge.X]++;
            if (group[edge.Y] != i) group[edge.Y] = i;
            else if (group[edge.Y] == i) count[edge.Y]++;
        }
    }
